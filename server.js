...
app.post('/reservations', authenticate, async (req, res, next) => {
...
app.get('/reservations', authenticate, async (req, res, next) => {
  try {
    const [rows] = await pool.query(
...
app.get('/reservations/:id', authenticate, async (req, res, next) => {
  const id = toBigInt(req.params.id);
  if (id === null) {
...
app.post('/reservations/:id/cancel', authenticate, async (req, res, next) => {
  const id = toBigInt(req.params.id);
  if (id === null) {
    return res.status(404).json({ message: 'Reservation not found' });
  }
